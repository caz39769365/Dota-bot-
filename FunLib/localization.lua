...
local Localization = {
    ...
    zh = {
        ...
        fret_default_diff_limit = '为了保障你的游戏体验，避免你因不熟悉高难度模式而可能面临的压倒性挑战 - 目前主机可选择的最高难度为：%d。主机需要至少 %d 次在难度 %d 中获胜后，才能选择难度 %d。正在设置难度为 %d',
    ...
    ru = {
        ...
        fret_default_diff_limit = 'Для вашего игрового опыта, если вы не знакомы с чрезмерно сложной игрой на высоком уровне сложности, максимальная сложность, которую может выбрать хост, составляет: %d. Хост должен выиграть на сложности %d как минимум %d раз(а), прежде чем выбрать сложность %d. Установка сложности на %d.',
    ...
    ja = {
        ...
        fret_default_diff_limit = '高難易度の圧倒的なゲーム体験にまだ慣れていない場合のため、現在ホストが選択できる最大難易度は: %d です。ホストは難易度 %d で少なくとも %d 回勝利する必要があり、その後難易度 %d を選択できます。難易度を %d に設定中です。',
